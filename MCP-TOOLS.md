...
MCP-tools 是一个专门为文件解析设计的 Docker 容器镜像，集成了所有 MCP 解析器依赖，包括：
...
```bash
cd backend
bash docker/build-mcp-tools.sh
```
...
mcp-tools    latest   xxxxxxxxxxxx   2 minutes ago    1.2GB
```
...
# 复制解析器模块
COPY app/services/mcp_parsers.py /app/
```
...
# 容器内执行解析
python3 -c "from mcp_parsers import parse_file; ..."
...
isMcpToolsContainerReady()
  ...
  → containerInfo.image === 'mcp-tools:latest'
```
...
```bash
# 1. 修改 Dockerfile
vim backend/docker/Dockerfile.mcp-tools

# 2. 重新构建
bash backend/docker/build-mcp-tools.sh
...
未来可以在 MCP-tools 中添加更多工具：

### 添加新依赖

编辑 `backend/docker/Dockerfile.mcp-tools`：
...
在 `backend/app/services/mcp_parsers.py` 中：
