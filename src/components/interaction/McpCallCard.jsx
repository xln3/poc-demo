...
export default function McpCallCard({ event }) {
  ...
  return (
    <div className="bg-surface-muted/50 border border-green-700/30 rounded-lg px-3 py-2 text-xs">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center gap-2 text-left"
      >
        <span className="text-green-400">🔌</span>
        <span className="font-mono text-on-surface">{serverName}/{toolName}</span>
