...
用法:
    python3 /app/mcp_parser_cli.py <file_path> <parsers_json>

示例:
    python3 /app/mcp_parser_cli.py /tmp/upload/doc.pdf '["pymupdf"]'
...
# 导入解析模块（位于同目录）
from mcp_parsers import parse_file, get_file_type


def main():
    if len(sys.argv) != 3:
        print(json.dumps([{"error": "用法: python3 mcp_parser_cli.py <file_path> <parsers_json>"}]))
        sys.exit(1)
